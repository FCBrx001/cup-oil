    ...
    export default{
        ...
        data(){
            return {
                //茂名站
                mmzd:{
                    STN02_00_PI012:'出站压力(MPa)',STN02_00_PI012A:'出站压力1(MPa)',
                    STN02_00_PI013A:'出站压力2(MPa)',STN02_00_DI002A:'出站密度(kg/m³)',STN02_00_FI101_102:'出站流量(t/h)',
                    STN02_00_TI002:'出站温度(℃)',

                    STN02_00_PIC016:'减压阀后压力(MPa)',
                    STN02_00_PI301:'主输泵入口压力(MPa)',STN02_00_PI009:'主输泵出口汇管压力1(MPa)',
                    STN02_00_PI010A:'主输泵出口汇管压力2(MPa)',STN02_00_PI010B:'主输泵出口汇管压力3(MPa)',
                    STN02_00_PI015A:'减压阀后压力15A(MPa)',STN02_00_PI015B:'减压阀后压力15B(MPa)'
                },
                ...
                value: [],
                checkedvalue:['STN02_00_PI012','STN02_00_PI012A'],
                
                start_value:'2023-03-3 00:00:00',
				over_value:'2023-03-6 00:00:00',  
        ...
        methods: { 
            ...
            datainit(){
                ...
                this.$axios({
                    ...
                    // console.log('x轴时间参数返回：',this.xdata)
                    this.drawchart()
                })
            },
            paramsdatainit(){
                ...
                this.$axios({
                    ...
                    for(var i=0;i<res.data.length;i++){
                        ...
                        // obj['name'] = res.data[i]
                        let name = Object.keys(this.mmzd)
                        if(name.includes(`${res.data[i]}`)){
                            obj['value'] = res.data[i]
                            obj['name'] = this.mmzd[`${res.data[i]}`]
                            obj['check'] = false
                            option.push(obj)
                        }   
                        
                    }
                    console.log('本站参数转换：',option)
            ...
            changeval(val,item){
                ...
                // this.seclectedparm = val
                // console.log('changeval选择',item)
                // if(item.check){
                //     this.seclectedparm.push(item.value)

                //     this.legenddata.push(item.name)
                // }else{
                //     this.seclectedparm.splice(this.seclectedparm.indexOf(item.value),1)
                // }
                // console.log('checkedgroup',this.checkedgroup)
                // console.log('已选参数：',this.seclectedparm)
                // console.log('已选参数中文名称：',this.legenddata)
            },
            suredraw(){
                this.sure()
            }
